...
func (db *DB) CreateProject(p *models.Project) error {
	...
	_, err := db.conn.Exec(`
		INSERT INTO projects (id, name, repo_path, base_branch, stage_branch, auto_test, auto_review, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, p.ID, p.Name, p.RepoPath, p.BaseBranch, p.StageBranch, p.AutoTest, p.AutoReview, p.CreatedAt, p.UpdatedAt)
	return err
}

func (db *DB) GetProjects() ([]models.Project, error) {
	rows, err := db.conn.Query(`
		SELECT id, name, repo_path, base_branch, stage_branch, auto_test, auto_review, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.RepoPath, &p.BaseBranch, &p.StageBranch, &p.AutoTest, &p.AutoReview, &p.CreatedAt, &p.UpdatedAt); err != nil {
			return nil, err
		}
...
func (db *DB) GetProject(id string) (*models.Project, error) {
	...
	`, id).Scan(&p.ID, &p.Name, &p.RepoPath, &p.BaseBranch, &p.StageBranch, &p.AutoTest, &p.AutoReview, &p.CreatedAt, &p.UpdatedAt)
...
func (db *DB) UpdateProject(p *models.Project) error {
	...
	_, err := db.conn.Exec(`
		UPDATE projects SET name = ?, repo_path = ?, base_branch = ?, stage_branch = ?, auto_test = ?, auto_review = ?, updated_at = ? WHERE id = ?
	`, p.Name, p.RepoPath, p.BaseBranch, p.StageBranch, p.AutoTest, p.AutoReview, p.UpdatedAt, p.ID)
	return err
}
